...
import static ru.wert.normic.controllers._plates.AbstractOpPlate.INTEGER_FORMAT;
...
//            ((IOpWithOperations)opData).getOperations().add(targetIndex, addedOpData);
    ...
    public void clearAll(Event e, boolean changeTitle, boolean clearSavesHistory) {
        ...
        if(clearSavesHistory) SAVES_HISTORY = new ArrayList<>(); //Обнуляем историю сохранений
        menu.addEmptyPlate();
    }
    ...
    private void deployProductSettings(ProductSettings settings) {
        ...
        CURRENT_LASER_MACHINE.set(
                settings.getLaserMachine() == null ?
                        DEFAULT_LASER_MACHINE :
                        settings.getLaserMachine());
