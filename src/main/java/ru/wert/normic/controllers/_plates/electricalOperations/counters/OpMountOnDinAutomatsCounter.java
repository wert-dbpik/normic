package ru.wert.normic.controllers._plates.electricalOperations.counters;
...
public class OpMountOnDinAutomatsCounter implements NormCounter{

    public OpData count(OpData data){
        OpMountOnDinAutomats opData = (OpMountOnDinAutomats)data;

        int avtomats = opData.getAvtomats(); //Количество автоматов
        boolean difficult = opData.isDifficult(); // Сложно устанавливать
        double k = difficult ? 1.3 : 1.0; //Коэффициент сложности

        //################################################################

        double timeOp =  avtomats * MOUNT_ON_DIN_AUTOMATS * k; //мин
