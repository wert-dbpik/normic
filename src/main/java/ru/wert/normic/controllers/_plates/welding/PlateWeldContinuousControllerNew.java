 ...
 */
public class PlateWeldContinuousControllerNew extends AbstractOpPlate {
    ...
    private OpWeldContinuousNew opData;
    ...
    public void countNorm(OpData data){
       opData = (OpWeldContinuousNew)data;

        countInitialValues();

        opData = (OpWeldContinuousNew) opData.getOpType().getNormCounter().count(data);
    ...
    public String helpText() {
        return String.format("Возможны два варианта указания данных для рассчета.\n\n" +
                        ...
                        "\tT зачистки - время зачистки дискретно зависит от L шва. и количества швов,\n" +
                        "\t\tтаким образом учитываются перевороты детали во время зачистки:\n\n" +
                        "\t\tL шва < 0.1        ->  T зачистки = 0.5 мин; \n" +
                        "\t\t0.1 <= L шва < 0.5 ->  T зачистки = 1.8 мин; \n" +
                        "\t\t0.5 <= L шва < 1.0 ->  T зачистки = 3.22 мин; \n" +
                        "\t\tL шва > 1000       ->  T зачистки = 3.22 мин x L шва." ,
                WELDING_SPEED);
