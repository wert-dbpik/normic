...
public class AppProperties {
    ...
    {
        if (TEST_VERSION)
            appConfigPath = homeDir + File.separator + "constantsTest.properties";
        else
            appConfigPath = homeDir + File.separator + "constants.properties";
    }
    ...
    private void createFileOfConnectionSettings(String appConfigPath) {
        log.debug("createFileOfConstants : constants.properties создается  ...");
        try {
            File dir = new File(homeDir);
            dir.mkdirs();

            File constants = new File(appConfigPath);
            constants.createNewFile();

            FileWriter writer = new FileWriter (constants);
            ...
            if(++attempt < 3) new AppProperties();
                ...
                Warning1.create("Ошибка!",
                        "Не удалось создать файл констант",
                        "Возможно, стоит защита от создания файлов,\n обратитесь к разработчику");
                e.printStackTrace();
            }

        }
        log.debug("createFileOfConstants : constants.properties успешно создан");
    }
